...
const gradientMap: Record<string, string> = {
  ...
  afro: "from-emerald-500/30 to-teal-500/30",
  electronic: "from-cyan-500/30 to-blue-500/30",
  rnb: "from-red-500/30 to-pink-500/30",
  lofi: "from-indigo-500/30 to-slate-500/30",
  "rock-indie": "from-orange-500/30 to-red-500/30",
  open: "from-amber-500/30 to-yellow-500/30",
  dj: "from-fuchsia-500/30 to-pink-500/30",
  reggae: "from-green-500/30 to-lime-500/30",
...
const CategoryDetail = () => {
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          <h1 className="font-display text-2xl font-bold">{t("categoryDetail.notFound")}</h1>
  ...
  return (
    <Layout>
      <SEOHead
        title={t("categoryDetail.seoTitle", { name: category.name })}
        description={category.description
          ? t("categoryDetail.seoDescWithDesc", { description: category.description, name: category.name })
          : t("categoryDetail.seoDescDefault", { name: category.name })}
        url={`/categories/${slug}`}
        jsonLd={[
          categoryJsonLd({ name: category.name, slug: category.slug, description: category.description }),
          breadcrumbJsonLd([
            { name: t("categoryDetail.breadcrumbHome"), url: "/" },
            { name: t("categoryDetail.breadcrumbCategories"), url: "/#categories" },
      ...
      >
        <div className="container py-16 md:py-24">
          ...
          >
            <ArrowLeft className="h-4 w-4" /> {t("categoryDetail.allCategories")}
          </Link>
        ...
        {category.sub_genres && category.sub_genres.length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.subGenresTitle")}</h2>
            </div>
            <p className="text-muted-foreground mb-5">{t("categoryDetail.subGenresDesc")}</p>
            <div className="flex flex-wrap gap-2.5">
              {category.sub_genres.map((sg) => (
                <Badge key={sg} variant="outline" className="px-4 py-2 text-sm font-medium border-primary/30 bg-primary/5">
                  {sg}
                </Badge>
              ))}
            </div>
        ...
        {category.mood_tags && category.mood_tags.length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.moodTitle")}</h2>
            </div>
            <p className="text-muted-foreground mb-5">{t("categoryDetail.moodDesc")}</p>
            <div className="flex flex-wrap gap-2.5">
              {category.mood_tags.map((mood) => (
                <span key={mood} className="inline-flex items-center rounded-full bg-secondary px-4 py-2 text-sm font-medium text-secondary-foreground">
                  {mood}
                </span>
              ))}
            </div>
        ...
        {category.production_tips && (category.production_tips as ProductionTip[]).length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.productionTipsTitle")}</h2>
            </div>
        ...
        {category.scoring_criteria && (category.scoring_criteria as ScoringCriterion[]).length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.scoringTitle")}</h2>
            </div>
            <p className="text-muted-foreground mb-5">{t("categoryDetail.scoringDesc")}</p>
              ...
              {(category.scoring_criteria as ScoringCriterion[]).map((sc) => (
                <div key={sc.criterion} className="rounded-xl border border-border bg-card p-5 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="inline-flex items-center rounded-full bg-primary/10 px-3 py-1 text-sm font-bold text-primary">{sc.weight}%</span>
                  </div>
                  <div className="w-full bg-muted rounded-full h-2">
                    <div className="bg-primary h-2 rounded-full transition-all" style={{ width: `${sc.weight}%` }} />
                  </div>
        ...
        {historyParagraphs.length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.historyTitle")}</h2>
            </div>
        ...
        {category.notable_artists && category.notable_artists.length > 0 && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-display text-2xl font-bold">{t("categoryDetail.notableArtistsTitle")}</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              {category.notable_artists.map((artist) => (
                <Badge key={artist} variant="secondary" className="px-4 py-2 text-sm font-medium">{artist}</Badge>
              ))}
            </div>
          </motion.section>
        )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col sm:flex-row gap-3">
          <Button asChild size="lg" className="rounded-full">
            <Link to={`/explore?category=${category.id}`}>
              ...
              {t("categoryDetail.viewSubmissions")}
            </Link>
          </Button>
          <Button asChild variant="outline" size="lg" className="rounded-full">
            <Link to="/compete">{t("categoryDetail.joinContest")}</Link>
          </Button>
        </motion.div>

        {/* Navigation between categories */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }} className="flex items-center justify-between border-t border-border pt-8">
          {prevCat ? (
            <Link to={`/categories/${prevCat.slug}`} className="group inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
              <ChevronLeft className="h-4 w-4 group-hover:-translate-x-0.5 transition-transform" />
              <span>{prevCat.name}</span>
            </Link>
          ) : <span />}
          {nextCat ? (
            <Link to={`/categories/${nextCat.slug}`} className="group inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
