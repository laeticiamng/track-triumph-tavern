...
interface ShareSheetProps { title: string; artistName: string; submissionId: string; }
...
export function ShareSheet({ title, artistName, submissionId }: ShareSheetProps) {
  ...
  const text = t("share.listen", { title, artist: artistName });
  ...
  return (
    <Drawer>
      <DrawerTrigger asChild>
        <button onClick={async (e) => { const shared = await handleNativeShare(); if (shared) e.preventDefault(); }} className="flex flex-col items-center gap-1 text-white/80 hover:text-white transition-colors">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white/15 backdrop-blur-sm"><Share2 className="h-5 w-5" /></div>
          <span className="text-[10px] font-medium">{t("share.share")}</span>
        </button>
      </DrawerTrigger>
      <DrawerContent>
        <DrawerHeader><DrawerTitle>{t("share.share")}</DrawerTitle></DrawerHeader>
        <div className="px-4 pb-6 space-y-3">
          ...
          {links.map((l) => (
            <Button key={l.label} variant="outline" className="w-full justify-start gap-3" asChild>
              <a href={l.href} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" />{l.label}</a>
            </Button>
          ))}
        </div>
      </DrawerContent>
