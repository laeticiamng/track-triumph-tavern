  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">Badges hebdomadaires</h3>
        <span className="text-xs text-muted-foreground">({badges.length})</span>
      </div>
      {badges.length === 0 ? (
        <p className="text-xs text-muted-foreground italic">Aucun badge gagné pour le moment. Continuez à voter !</p>
      ) : (
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className={`flex items-center justify-center rounded-full bg-gradient-to-br ${config.color} ${sizeClasses} cursor-default shadow-md`}
...
/** Inline badge pills for headers / compact views */
export function BadgePills({ userId }: { userId?: string }) {
  const { badges } = useWeeklyBadges(userId);
  if (badges.length === 0) return null;

  // Show unique badge types from latest week only
